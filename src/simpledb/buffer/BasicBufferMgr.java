...
//	TODO
	private Buffer chooseUnpinnedBuffer_Clock() { 
		for (int i = 0; i<bufferpool.length; i++) {
			int bufferIndex = (i + getLastReplacedPageIndex() + 1) % bufferpool.length;
			Buffer buff = bufferpool[bufferIndex];
			if (!buff.isPinned()) {
				setLastReplacedPageIndex(bufferIndex);
				return buff;
			}
		}
		return null;
	...
	public int getLastReplacedPageIndex() {
		return lastReplacedPageIndex;
	}

	public void setLastReplacedPageIndex(int lastReplacedPageIndex) {
		this.lastReplacedPageIndex = lastReplacedPageIndex;
	}
