...
public class RecordFile {
   ...
   public int getInt(String fldname) {
	   ...
	   BasicRecordStats brsCurrentStats = getStatsBlock(currentRid);
	   brsCurrentStats.incrementalReadFieldRecord();
	   if(!this.lastReadRecord.equals(currentRid)) {
		   brsCurrentStats.incrementalReadRecord();
   ...
   public void setInt(String fldname, int val) {
	   RID currentRid = this.currentRid();
	   BasicRecordStats brsCurrentStats = getStatsBlock(currentRid);
	   brsCurrentStats.incrementalWrittenFieldRecord();
	   if(!this.lastWrittenRecord.equals(currentRid)) {
		   brsCurrentStats.incrementalWrittenRecord();
   ...
   public BasicRecordStats getStatsBlock(RID curRid) {
	   BasicRecordStats brsCurrStats = statsRecord.get(curRid);
	   if(brsCurrStats == null) {
		   brsCurrStats = new BasicRecordStats();
		   this.statsRecord.put(curRid, brsCurrStats);
	   }
	   return brsCurrStats;
   }
